...
def setButtonFontWeightString( button, isBold ):
	...
	try:
		...
		button['font'] = ('Times', 10, 'bold italic')
	except:
		pass
...
class StringSetting:
	...
	def addToDialog( self, gridPosition ):
		...
		gridPosition.increment()
		self.label = Tkinter.Label( gridPosition.master, text = self.name, font=('Times', 10, 'italic'))
	...
	def addToMenuFrameable( self, repositoryMenu ):
		...
		repositoryMenu.add_cascade( label = titleFromName, font=('Times', 10, 'italic'), menu = helpWindowMenu, underline = 0 )
		if self.name in self.repository.frameList.value:
			helpWindowMenu.add_command( label = 'Remove from Window', font=('Times', 10, 'italic'), command = self.removeFromWindow )
		else:
			helpWindowMenu.add_command( label = 'Add to Window', font=('Times', 10, 'italic'), command = self.addToWindow )
		helpWindowMenu.add_separator()
		helpWindowMenu.add_command( label = 'Help', font=('Times', 10, 'italic'), command = HelpPage().getOpenFromDocumentationSubName( self.repository.fileNameHelp + '#' + titleFromName ) )
...
class BooleanSetting( StringSetting ):
	...
	def addToDialog( self, gridPosition ):
		"Add this to the dialog."
		gridPosition.increment()
		self.checkbutton = Tkinter.Checkbutton( gridPosition.master, command = self.toggleCheckbutton, text = self.name, font=('Times', 10, 'italic') )
...
class DisplayToolButton:
	...
	def addToDialog( self, gridPosition ):
		"Add this to the dialog."
		self.displayButton = Tkinter.Button( gridPosition.master, activebackground = 'black', activeforeground = 'white', text = getEachWordCapitalized( self.name ), font=('Times', 10, 'italic'), command = self.displayDialog )
...
class FileHelpMenuBar:
	def __init__( self, root ):
		...
		self.menuBar.add_cascade( label = "File", font=('Times', 10, 'italic'), menu = self.fileMenu, underline = 0 )
	...
	def addMenuToMenuBar( self, labelText, menu ):
		...
		for underlineLetterIndex in xrange( len( lowerLabelText ) ):
			...
			if underlineLetter not in self.underlineLetters:
				...
				self.menuBar.add_cascade( label = labelText, font=('Times', 10, 'italic'), menu = menu, underline = underlineLetterIndex )
				return
...
class FloatSpin( FloatSetting ):
	...
	def addToMenuFrameable( self, repositoryMenu ):
		...
		helpWindowMenu.add_command( label = 'Increase' + changeString, font=('Times', 10, 'italic'), command = self.increase )
		helpWindowMenu.add_command( label = 'Decrease' + changeString, font=('Times', 10, 'italic'), command = self.decrease )
		helpWindowMenu.add_separator()
		helpWindowMenu.add_command( label = 'Help', font=('Times', 10, 'italic'), command = HelpPage().getOpenFromDocumentationSubName( self.repository.fileNameHelp + '#' + titleFromName ) )
...
class HelpPage:
	...
	def addToDialog( self, gridPosition ):
		...
		self.displayButton = Tkinter.Button( gridPosition.master, activebackground = 'black', activeforeground = 'white', command = self.openPage, font=('Times', 10, 'italic'), text = capitalizedName )
	...
	def addToMenu( self, repositoryMenu ):
		"Add this to the repository menu."
		repositoryMenu.add_command( label = getTitleFromName( self.name ), font=('Times', 10, 'italic'), command = self.openPage )
...
#	def ShowHelp( TextBuffer ):
#		sub_win = Toplevel()
#		c0 = Canvas(sub_win, width = 150, height = 150)
		#c0.create_rectangle(10, 10, 140, 140, fill = 'green', stipple = 'gray25')
#		c0.create_text(75, 75, text = TextBuffer, font = ('FixedSys', 14))
#		c0.pack()
...
class LabelHelp:
	...
	def __init__( self, fileNameHelp, master, name, widget ):
		...
		self.popupMenu.add_command( label = 'Help', font=('Times', 10, 'italic'), command = HelpPage().getOpenFromDocumentationSubName( fileNameHelp + '#' + titleFromName ) )
...
class MenuButtonDisplay:
	...
	def addToMenu( self, repositoryMenu ):
		...
		repositoryMenu.add_cascade( label = getTitleFromName( self.name ), font=('Times', 10, 'italic'), menu = self.menu )
	...
	def addToMenuFrameable( self, repositoryMenu ):
		...
		titleFromName = getTitleFromName( self.name )
		self.addToMenu( repositoryMenu )
		self.menu.add_command( label = 'Help', font=('Times', 10, 'italic'), command = HelpPage().getOpenFromDocumentationSubName( self.repository.fileNameHelp + '#' + titleFromName ) )
...
class MenuRadio( BooleanSetting ):
	...
	def addToSubmenu(self):
		...
		menu.add_radiobutton( label = self.name, font=('Times', 10, 'italic'), command = self.clickRadio, value = self.name, variable = self.menuButtonDisplay.radioVar )
...
class PluginFrame:
	...
	def createFrame( self, gridPosition ):
		...
		gridVertical.master = Tkinter.LabelFrame( gridPosition.master, borderwidth = 3, relief = 'sunken')
		...
		if executeTitle != None:
			executeButton = Tkinter.Button( gridVertical.master, activebackground = 'black', activeforeground = 'blue', text = executeTitle, font=('Times', 10, 'italic'), command = gridVertical.frameGridVertical.execute )
			executeButton.grid( row = gridVertical.row, column = gridVertical.column, sticky = Tkinter.W )
			gridVertical.column += 1
		self.helpButton = Tkinter.Button( gridVertical.master, activebackground = 'black', activeforeground = 'white', text = "?", font=('Times', 10, 'italic'), command = HelpPageRepository( gridVertical.repository ).openPage )
...
class PluginGroupFrame( PluginFrame ):
	...
	def createFrame( self, gridPosition ):
		...
		executeTitle = gridVertical.repository.executeTitle
		if executeTitle != None:
			executeButton = Tkinter.Button( gridVertical.master, activebackground = 'black', activeforeground = 'blue', text = executeTitle, font=('Times', 10, 'italic'), command = gridVertical.execute )
			executeButton.grid( row = gridVertical.row, column = gridVertical.column, sticky = Tkinter.W )
			gridVertical.column += 1
		self.helpButton = Tkinter.Button( gridVertical.master, activebackground = 'black', activeforeground = 'white', text = "?", font=('Times', 10, 'italic'), command = HelpPageRepository( gridVertical.repository ).openPage )
		self.helpButton.grid( row = gridVertical.row, column = gridVertical.column, sticky = Tkinter.W )
...
class Radio( BooleanSetting ):
	...
	def createRadioButton( self, gridPosition ):
		...
		self.radiobutton = Tkinter.Radiobutton( gridPosition.master, command = self.clickRadio, text = self.name, font=('Times', 10, 'italic'), value = self.name, variable = self.latentStringVar.getVar() )
...
class RadioCapitalized( Radio ):
	...
	def createRadioButton( self, gridPosition ):
		"Create the radio button."
		capitalizedName = getEachWordCapitalized( self.name )
		self.radiobutton = Tkinter.Radiobutton( gridPosition.master, command = self.clickRadio, text = capitalizedName, font=('Times', 10, 'italic'), value = self.name, variable = self.latentStringVar.getVar() )
...
class RadioCapitalizedButton( Radio ):
	...
	def createRadioButton( self, gridPosition ):
		...
		capitalizedName = getEachWordCapitalized( self.name )
		self.radiobutton = Tkinter.Radiobutton( gridPosition.master, command = self.clickRadio, text = capitalizedName, font=('Times', 10, 'italic'), value = self.name, variable = self.latentStringVar.getVar() )
		self.displayButton = Tkinter.Button( gridPosition.master, activebackground = 'black', activeforeground = 'white', font=('Times', 10, 'italic'), text = capitalizedName, command = self.displayDialog )
...
class RadioPlugin( RadioCapitalized ):
	...
	def addToDialog( self, gridPosition ):
		"Add this to the dialog."
		self.createRadioButton( gridPosition )
		self.radiobutton['background'] = 'gray20'
		self.radiobutton['activeforeground'] = 'magenta'
		self.radiobutton['selectcolor'] = 'dimgray'
		self.radiobutton['padx'] = 20
		self.radiobutton['pady'] = 5
		self.radiobutton['borderwidth'] = 4
	...
	def incrementGridPosition( self, gridPosition ):
		...
		gridPosition.incrementGivenNumberOfColumns( 5 )
		self.radiobutton.grid( row = gridPosition.row, column = gridPosition.column, sticky = Tkinter.N + Tkinter.S + Tkinter.W + Tkinter.E )
...
class ToolDialog:
	...
	def addPluginToMenu( self, menu, path ):
		...
		self.path = path
		menu.add_command( label = getEachWordCapitalized( name ) + '...', font=('Times', 10, 'italic'), command = self.display )
...
class RepositoryDialog:
	def __init__( self, repository, root ):
		...
		root.tk_setPalette("background", 'gray15', "foreground", 'gray')
	...
	def addButtons( self, repository, root ):
		...
		if repository.executeTitle != None:
			executeButton = Tkinter.Button( root, activebackground = 'black', activeforeground = 'blue',bd='4',padx=20,pady=10, font=('Times', 10, 'italic'), text = repository.executeTitle, command = self.gridPosition.execute )
			executeButton.grid( row = self.gridPosition.row, column = columnIndex, columnspan = 2, sticky = Tkinter.W )
			columnIndex += 2
		self.helpButton = Tkinter.Button( root, activebackground = 'black', activeforeground = 'white',bd='4',padx=20,pady=10, text = "?", font=('Times', 10, 'italic'), command = HelpPageRepository(self.repository).openPage )
		...
		columnIndex += 6
		cancelButton = Tkinter.Button( root, activebackground = 'black', activeforeground = 'orange', command = self.cancel, fg = 'orange',bd='4',padx=20,pady=10, font=('Times', 10, 'italic'), text = 'Cancel')
		cancelButton.grid( row = self.gridPosition.row, column = columnIndex )
		columnIndex += 1
		self.saveButton = Tkinter.Button( root, activebackground = 'black', activeforeground = 'darkgreen', command = saveCommand, fg = 'darkgreen', bd='4',padx=20,pady=10, font=('Times', 10, 'italic'), text = saveText )
